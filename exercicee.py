...
# Fonction pour supprimer un todo avec confirmation
def supprimer_todo():
    if not todos:
    ...
    try:
        ...
        todo = todos[choix]
        confirmation = input(f"Êtes-vous sûr de vouloir supprimer le todo '{todo['titre']}' ? (o/n) : ").lower()
        if confirmation == 'o':
            todos.pop(choix)
            print(f"Le todo '{todo['titre']}' a été supprimé avec succès.")
        else:
